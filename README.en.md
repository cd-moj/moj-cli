...
| `rounds problems <slug> [ls\|set <id,id…>\|add <id>\|rm <letter>]` | Sets the problems of each round. They go live when the round is promoted. | `moj-contest -c lab1 rounds problems aq set apc#a,apc#b` |
| `rounds promote [--force]` | Archives the live round and puts the next one live. It refuses with a job in flight, a pending verdict or an open review. `rounds ls` lists the blockers. It asks for the contest id. | `moj-contest -c lab1 rounds promote` |
